...
var registerRoutes = []struct {
	...
	// explicit route /explicitly should replace the implicit ServeMux redirect
	{"/explicitly/", "explicitly tree"},
	{"/explicitly", "explicitly leaf"},
}
...
func TestHandler(t *testing.T) {
	...
	for _, route := range registerRoutes {
		mux.Handle(route.pattern, stringHandler(route.message))
	}

	for _, ht := range handlerTestCases {
...
func registerVerbRoutes(mux *ServeMux) {
	...
	// multiple allowed verbs
	getOrPostOnly := stringHandler("get-or-post-only")
	mux.Get("/get-or-post-only", getOrPostOnly)
	mux.Post("/get-or-post-only", getOrPostOnly)
	postOrPutOnly := stringHandler("post-or-put-only")
	mux.Post("/post-or-put-only", postOrPutOnly)
	mux.Put("/post-or-put-only", postOrPutOnly)
...
var verbTestCases = []struct {
	...
	// multiple allowed verbs
	{"GET", "/get-or-post-only", 200, "/get-or-post-only"},
	{"POST", "/get-or-post-only", 200, "/get-or-post-only"},
	{"PUT", "/get-or-post-only", 404, ""},
	{"GET", "/post-or-put-only", 404, ""},
	{"POST", "/post-or-put-only", 200, "/post-or-put-only"},
	{"PUT", "/post-or-put-only", 200, "/post-or-put-only"},
	{"DELETE", "/post-or-put-only", 404, ""},
	// method rules can be applied on tree patterns, just as on leaf patterns
	{"GET", "/tree/", 404, ""},
	{"POST", "/tree/", 404, ""},
	{"PUT", "/tree/", 404, ""},
	{"DELETE", "/tree/", 200, "/tree/"},
	// explicit routes override implicit redirects, wrong method results in a
	// 404 like any other explicit route, implicit redirect was removed
	{"POST", "/tree", 200, "/tree"},
	{"DELETE", "/tree", 404, ""},
...
func TestHandlerMethods(t *testing.T) {
	mux := NewServeMux()
	registerVerbRoutes(mux)

	for _, ht := range verbTestCases {
		r := newRequest(ht.method, ht.url)
		_, pattern := mux.Handler(r)
		if pattern != ht.pattern {
			t.Errorf("%s %s -> pattern %s, want %s", ht.method, ht.url, pattern, ht.pattern)
